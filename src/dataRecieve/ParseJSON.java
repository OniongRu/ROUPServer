...
public class ParseJSON
{
    ...
    public static boolean RegisterClSender(String gsonClient, int privilege)
    {
        try
        {
            ...
            String url = Properties.getInstance().getDatabaseConnection();
            String login = Properties.getInstance().getLogin();
            String password = Controller.getInstance().getPassword();

            var conn = DriverManager.getConnection(url, login, password);
